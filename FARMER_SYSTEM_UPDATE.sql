-- ========================================
-- ACTUALIZACIÓN SISTEMA DE AGRICULTORES
-- ========================================

-- 1. Crear tabla farmer_applications (si no existe)
...
-- 2. Función para aprobar una solicitud y crear el agricultor
...
-- 3. Función para rechazar una solicitud
...
-- 4. Función para obtener estadísticas del dashboard
...
BEGIN
  RETURN (
    ...
    FROM products 
    WHERE farmer_id = get_farmer_products.farmer_id
    AND status = 'published'
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 6. Trigger para updated_at en farmer_applications
...
-- 9. Datos de prueba para administrador (opcional)
INSERT INTO admins (email, password_hash, first_name, last_name, role, permissions, is_active)
VALUES (
  'admin@saboratierra.com',
  '$2b$10$example.hash.here', -- Cambiar por hash real
  'Admin',
  'Sistema',
  'superadmin',
  ARRAY['manage_farmers', 'manage_products', 'manage_orders', 'view_analytics'],
  true
) ON CONFLICT (email) DO NOTHING;

-- ========================================
-- ESQUEMA ACTUALIZADO COMPLETADO
-- ========================================
